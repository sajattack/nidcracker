...
fn main() {
    ...
    //let module_names: Vec<&str> = module_names_string.lines().collect();

    let module_name = "sceSysreg";

    let wordlist_string = read_to_string("wordlist.txt").unwrap();
    let wordlist: Vec<&str> = wordlist_string.lines().collect();

    let unknown_nids_string = read_to_string("unknown_nids.txt").unwrap();
    let unk_nids_iter = unknown_nids_string.lines();
    let unk_nids: Vec<u32> = unk_nids_iter.map(|s| u32::from_str_radix(s, 16).unwrap()).collect();
